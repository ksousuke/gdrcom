...
static struct ibv_mr *client_metadata_mr = NULL, *server_src_mr = NULL, *server_dst_mr = NULL;
...
#define TOTALSIZE (64) /* Total memory region size (BUFFSIZE*2) */
#define BUFFSIZE (32) /* Memory region size (RDMA Write or RDMA Read) */

/* This function checks src buffer */
static void check_src_buffer()
{
	printf("src: '%s'\n", src);
}

/* This function checks dst buffer */
static void check_dst_buffer()
{
	printf("dst: '%s'\n", dst);
}

/* This function sets a string to buffers */
static void set_buff(char *dst, char *src)
{
	strncpy(dst, src, strlen(src));
}

/* This function does RDMA Write from src to remote buffer */
int server_rdma_write(int size, int sp)
{
	int ret = -1;
	...
	rdma_write_sge.addr = (uint64_t)server_src_mr->addr;
	rdma_write_sge.length = size;
	rdma_write_sge.lkey = server_src_mr->lkey;
	...
	rdma_write_wr.wr.rdma.remote_addr = client_metadata_attr.address+sp;
	rdma_write_wr.wr.rdma.rkey = client_metadata_attr.stag.local_stag;
	rdma_write_wr.send_flags = IBV_SEND_SIGNALED;
...
/* This function does RDMA Read from remote buffer to dst */
int server_rdma_read(int size, int sp)
{
	int ret = -1;
	struct ibv_send_wr *bad_wr = NULL;
	struct ibv_wc wc[1];
	...
	rdma_read_sge.addr = (uint64_t)server_dst_mr->addr;
	rdma_read_sge.length = size;
	rdma_read_sge.lkey = server_dst_mr->lkey;
	...
	rdma_read_wr.wr.rdma.remote_addr = client_metadata_attr.address+sp;
	rdma_read_wr.wr.rdma.rkey = client_metadata_attr.stag.local_stag;
...
static int start_rdma_server(struct sockaddr_in *server_addr)
{
	...
	printf("Server is listening successfully port: %d \n", ntohs(server_addr->sin_port));
...
static void setup_src_dst_buffer()
{
	struct ibv_wc wc[1];
	int ret = -1;

	/* At this point we are expecting 1 work completion. One for our
	 * send and one for recv that we will get from the client for
	...
	server_src_mr = rdma_buffer_register(pd,
			src,
	...
	// Prepare dst buffer
	server_dst_mr = rdma_buffer_register(pd,
			dst,
			sizeof(char)*TOTALSIZE,
...
static int disconnect_and_cleanup()
{
	...
	/* active disconnect from the server side */
...
{
	int ret, option;
	char src_string[] = "input", *s = src_string;
	char dst_string[] = "output", *d = dst_string;
	...
	/* Set src and dst */
	src = calloc(sizeof(char)*BUFFSIZE , 1);
	if (!src) {
		rdma_error("Failed to allocate memory : -ENOMEM\n");
		return -ENOMEM;
	}
	dst = calloc(sizeof(char)*BUFFSIZE , 1);
	if (!dst) {
	...
	/* Connect src and dst with buffers */
	setup_src_dst_buffer();

	/* Copy strings to write buffer */
	set_buff((char *)server_src_mr->addr, (char *)s);

	/* Write strings to RDMA buffer*/
	ret = server_rdma_write(5, 0);
	if (ret) {
	  rdma_error("Failed to finish RDMA write, ret = %d \n", ret);
	  return ret;
	}

	/* Wait RDMA Write and write strings */
	while(1){
	        /* Read strings which are written by GPU */
	        ret = server_rdma_read(6, BUFFSIZE);
		if (ret) {
		        rdma_error("Failed to finish RDMA write, ret = %d \n", ret);
			return ret;
		}
		/* Check strings */
	        if(memcmp((void *)server_dst_mr->addr, (void *)d, 6)==0){
			break;
		}
	}
